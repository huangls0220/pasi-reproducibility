...
from src.matching import (
    _coverage_first_milp,
    coverage_first_matching,
    lagrangian_matching,
    milp_matching,
)
...
class TestCoverageFirstMILPFallback:
    def test_large_instance_preserves_lexicographic_objective(self):
        ...
        assert result["solver"] == "full_cardinality_hungarian_exact"
        _check_assignment(P, T, result["selected_idx"])

    def test_fast_path_matches_two_stage_milp_oracle(self):
        rng = np.random.default_rng(20260916)
        for _ in range(3):
            n = 13
            P, T = np.meshgrid(np.arange(n), np.arange(n), indexing="ij")
            P = P.ravel(); T = T.ravel()
            C = rng.uniform(0.1, 2.0, size=len(P))
            V = rng.uniform(0.1, 5.0, size=len(P))
            budget = float(n * 2.0)
            fast = coverage_first_matching(P, T, V, C, n, n, budget)
            oracle = _coverage_first_milp(P, T, V, C, n, n, budget)
            assert fast["solver"] == "full_cardinality_hungarian_exact"
            assert fast["maximum_cardinality"] == oracle["maximum_cardinality"]
            assert fast["total_cost"] == pytest.approx(oracle["total_cost"], abs=1e-8)
            _check_assignment(P, T, fast["selected_idx"])

    def test_k_cardinality_solver_matches_milp_on_sparse_graphs(self):
        rng = np.random.default_rng(72)
        for trial in range(10):
            n_p, n_t = 13, 14
            P, T, V, C = _random_instance(rng, n_p, n_t, feas_frac=0.55)
            budget = float(rng.uniform(2.0, 12.0))
            exact = coverage_first_matching(P, T, V, C, n_p, n_t, budget)
            oracle = _coverage_first_milp(P, T, V, C, n_p, n_t, budget)
            assert exact["maximum_cardinality"] == oracle["maximum_cardinality"], trial
            assert exact["total_cost"] == pytest.approx(oracle["total_cost"], abs=1e-7), trial
            _check_assignment(P, T, exact["selected_idx"])
    ...
    def test_large_instance_respects_budget_before_payment_tie_break(self):
        ...
        assert result["solver"] == "k_cardinality_hungarian_exact"
        _check_assignment(P, T, result["selected_idx"])
