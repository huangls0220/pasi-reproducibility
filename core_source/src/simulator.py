...
class Simulator:
    ...
    def _synchronize_observable_state_reset(self) -> None:
        """Expose a declared observable interruption to contract design.

        The environment event always changes the runtime path state.  When the
        configuration declares that interruption observable, the platform's
        state used for the same-slot offer must receive the same reset.  This
        is path-state observation, not response-function or cost observation.
        """
        if self.state_reset_config is None:
            return
        if not bool(self.state_reset_config.get("observable_to_platform", False)):
            return
        ids = self.reset_provider_ids
        if ids is None or len(ids) == 0:
            return
        self.H_design_state[ids] = self.H_true[ids]
        self._obs_last[ids] = self.H_true[ids]
        self._obs_cache_slot = None
        self._obs_cache = self.H_design_state.copy()
        ...
        for slot in slots:
            ...
            if self.state_reset_config is not None and not self._reset_applied:
                ...
                if evt["applied"]:
                    ...
                    self._synchronize_observable_state_reset()
