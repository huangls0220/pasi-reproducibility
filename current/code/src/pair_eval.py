...
from .robust_payment import objective_ir_reserve
...
def evaluate_pairs(mech: MechanismSpec, ctx: dict) -> dict:
    ...
    # Optional PASI-U uncertainty envelope. Relative bounds are
    ...
    if mech.contract_mode == "p1":
        ...
        # Interior Prelec minimum exists for zeta>1.
        zz = np.where(zeta_d > 1.0, zeta_d, 2.0)
        stationary = np.exp(-np.exp(-np.log(zz)/(zz-1.0)))
        p_I = np.clip(stationary, p_L, p_max)
        D_I = Lambda / np.maximum(W_vec(p_I, zeta_d), _EPS)
        use_I = ((zeta_d > 1.0) & (D_I <= D_bar+1e-9)
                 & (p_I*D_I < p*D-1e-12))
        p = np.where(use_I, p_I, p)
        D = np.where(use_I, D_I, D)
        zero = Lambda <= 0
    ...
    a_star = _bisect_root(Gamma_t, kappa_t, alpha_t, beta_t, L, np.zeros(n))

    # Always recompute from design inputs. A stale equality flag must not
    # introduce current response/cost/state into an offer or feasibility.
    Gamma_d = W_vec(p, zeta_d) * D + omega_d * H_d
    a_star_d = _bisect_root(Gamma_d, kappa_d, alpha_d, beta_d, L, np.zeros(n))
    ...
    if mech.base_mode == "fixed":
        ...
        if robust_enabled:
            # A perceived best response need not maximize objective utility.
            # Use a certified continuous-envelope reserve, not target-only IR.
            b_floor, _ = objective_ir_reserve(
                U_out, W_vec(p, zeta_d)*D, p*D, omega_d*H_lo,
                kappa_lo, kappa_hi, alpha_hi, beta_hi, L)
            b = np.maximum(b, b_floor)

    # These fields alone are allowed to enter allocation and budget checks.
    # For robust offers the response-independent upper bound is sufficient,
    # not necessarily the exact worst-case induced payment or its optimum.
    design_cost = b + p*D*g_star_d
    decision_cost = (b + p*D*g_vec(np.ones(n), kappa_hi)
                     if robust_enabled else design_cost)
    design_ir_ok = (b+(p*D+omega_d*H_d)*g_star_d
                    - C_vec(a_star_d, alpha_d, beta_d, L)+1e-8 >= U_out)
    ...
    decision_value = V*q_bar*g_star_d-decision_cost+reinf
    ...
    return {
        ...
        "design_expected_contract_cost": _z(design_cost),
        "decision_contract_cost": _z(decision_cost),
        "decision_pair_value": _z(decision_value),
        "decision_quality": _z(q_bar*g_star_d),
        "decision_ir_ok": design_ir_ok | ~ok,
